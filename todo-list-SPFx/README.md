...
* Use the team site in SharePoint to create a List.
    ...
    > *This is because each Microsoft Teams has a corresponding team site and Teams Group Tab can automatically detect the target SharePoint team site. A "To Do List" Sharepoint List is required in each teams site if the group tab is added to the corresponding Team. If you want to create a List in other subsite, manual steps to change the site url in* *[./SPFx/src/webparts/todoList/components/SharePointListManager.ts](./SPFx/src/webparts/todoList/components/SharePointListManager.ts) are required.*
