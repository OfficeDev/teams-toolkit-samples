...
1. Replace the placeholder `<webhook-url>` with the webhook URL in `./src/index.ts`
    ...
    ```
1. Execute the command `npm install`, `npm run build` and `npm run start`. 
...
1. [Add an incoming webhook in Teams](https://docs.microsoft.com/en-us/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook#create-an-incoming-webhook-1) and replace the placeholder `<webhook-url>` with the webhook URL in `./src/index.ts`.
1. Open Debug View (`Ctrl+Shift+D`) and select "Attach to Incoming Webhook" in dropdown list.

## Architecture
### Code structure
- You will find the incoming webhook code in: [src/](src/)
- You will find the adaptive card template in [src/adaptiveCards](src/adaptiveCards)
