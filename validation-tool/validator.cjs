...
`);t=n[e+1]}return e}rn.FOLD_BLOCK=Bs;rn.FOLD_FLOW=m0;rn.FOLD_QUOTED=Mi;rn.foldFlowLines=g0});var on=g(qf=>{"use strict";var qe=re(),Ye=Mf(),xi=(n,e)=>({indentAtStart:e?n.indent.length:n.indentAtStart,lineWidth:n.options.lineWidth,minContentWidth:n.options.minContentWidth}),$i=n=>/^(%|---|\.\.\.)/m.test(n);function y0(n,e,t){if(!e||e<0)return!1;let i=e-t,r=n.length;if(r<=i)return!1;for(let s=0,o=0;s<r;++s)if(n[s]===`
`){if(s-o>i)return!0;if(o=s+1,r-o<=i)return!1}return!0}function sn(n,e){let t=JSON.stringify(n);if(e.options.doubleQuotedAsJSON)return t;let{implicitKey:i}=e,r=e.options.doubleQuotedMinMultiLineLength,s=e.indent||($i(n)?"  ":""),o="",a=0;for(let l=0,c=t[l];c;c=t[++l])if(c===" "&&t[l+1]==="\\"&&t[l+2]==="n"&&(o+=t.slice(a,l)+"\\ ",l+=1,a=l,c="\\"),c==="\\")switch(t[l+1]){case"u":{o+=t.slice(a,l);let u=t.substr(l+2,4);switch(u){case"0000":o+="\\0";break;case"0007":o+="\\a";break;case"000b":o+="\\v";break;case"001b":o+="\\e";break;case"0085":o+="\\N";break;case"00a0":o+="\\_";break;case"2028":o+="\\L";break;case"2029":o+="\\P";break;default:u.substr(0,2)==="00"?o+="\\x"+u.substr(2):o+=t.substr(l,6)}l+=5,a=l+1}break;case"n":if(i||t[l+2]==='"'||t.length<r)l+=1;else{for(o+=t.slice(a,l)+`
...
`)||/[ \t]\n|\n[ \t]/.test(n))return sn(n,e);let t=e.indent||($i(n)?"  ":""),i="'"+n.replace(/'/g,"''").replace(/\n+/g,`$&
...
+(?!
|$)`,"g")}catch{Hs=/\n+(?!\n|$)/g}function qi({comment:n,type:e,value:t},i,r,s){let{blockQuote:o,commentString:a,lineWidth:l}=i.options;if(!o||/\n[\t ]+$/.test(t)||/^\s*$/.test(t))return At(t,i);let c=i.indent||(i.forceBlockIndent||$i(t)?"  ":""),u=o==="literal"?!0:o==="folded"||e===qe.Scalar.BLOCK_FOLDED?!1:e===qe.Scalar.BLOCK_LITERAL?!0:!y0(t,l,c.length);if(!t)return u?`|
`:`>
...
`))return qi(n,e,t,i);if($i(s)){if(l==="")return e.forceBlockIndent=!0,qi(n,e,t,i);if(a&&l===c)return At(s,e)}let f=s.replace(/\n+/g,`$&
...
`+j:U.comment=j,v.comment=v.comment.substring(j.length+1)}}if(!o&&!R&&!v.found){let j=q?n(t,q,v,r):e(t,v.end,R,null,v,r);c.items.push(j),f=j.range[2],Do(q)&&r(j.range,"BLOCK_IN_FLOW",Fo)}else{let j=v.end,U=b?n(t,b,v,r):e(t,j,I,null,v,r);Do(b)&&r(U.range,"BLOCK_IN_FLOW",Fo);let k=id.resolveProps(R??[],{flow:a,indicator:"map-value-ind",next:q,offset:U.range[2],onError:r,startOnNewline:!1});if(k.found){if(!o&&!v.found&&t.options.strict){if(R)for(let $ of R){if($===k.found)break;if($.type==="newline"){r($,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line");break}}v.start<k.found.offset-1024&&r(k.found,"KEY_OVER_1024_CHARS","The : indicator must be at most 1024 chars after the start of an implicit flow sequence key")}}else q&&("source"in q&&q.source&&q.source[0]===":"?r(q,"MISSING_CHAR",`Missing space after : in ${a}`):r(k.start,"MISSING_CHAR",`Missing , or : between ${a} items`));let F=q?n(t,q,k,r):k.found?e(t,k.end,R,null,k,r):null;F?Do(q)&&r(F.range,"BLOCK_IN_FLOW",Fo):k.comment&&(U.comment?U.comment+=`
`+k.comment:U.comment=k.comment);let x=new qT.Pair(U,F);if(t.options.keepSourceTokens&&(x.srcToken=A),o){let $=c;DT.mapIncludes(t,$.items,U)&&r(j,"DUPLICATE_KEY","Map keys must be unique"),$.items.push(x)}else{let $=new nd.YAMLMap(t.schema);$.flow=!0,$.items.push(x),c.items.push($)}f=F?F.range[2]:k.end}}let d=o?"}":"]",[_,...E]=i.end,m=f;if(_&&_.source===d)m=_.offset+_.source.length;else{let p=a[0].toUpperCase()+a.substring(1),A=u?`${p} must end with a ${d}`:`${p} in block collection must be sufficiently indented and end with a ${d}`;r(f,u?"MISSING_CHAR":"BAD_INDENT",A),_&&_.source.length!==1&&E.unshift(_)}if(E.length>0){let p=$T.resolveEnd(E,m,t.options.strict,r);p.comment&&(c.comment?c.comment+=`
