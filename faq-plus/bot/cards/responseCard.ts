 ...
 * @return The response card to append to a message as an attachment.
 */
export function getResponseCard(
  response: QnASearchResult,
  userQuestion: string,
...
function buildResponseCardBody(
  response: QnASearchResult,
  userQuestion: string,
  answer: string,
  payload: ResponseCardPayload
  ...
  if (response?.context.prompts.length > 0) {
    const previousQuestions = buildListOfPreviousQuestions(
      response.id,
      userQuestion,
      answer,
      payload
    ...
    for (let i = 0; i < response.context.prompts.length; i++) {
      ...
      image.url =
        "https://github.com/OfficeDev/microsoft-teams-apps-faqplus/blob/master/Source/Microsoft.Teams.Apps.FAQPlusPlus/wwwroot/Content/Followupicon.png"; // todo: host in bot app
...
function buildListOfPreviousQuestions(
  id: number,
  userQuestion: string,
  answer: string,
  payload: ResponseCardPayload
