...
async function main() {
  ...
  const qnaMakerClient = new QnAMakerClient(
    new CognitiveServicesCredentials(process.env.QNAMAKERSUBSCRIPTIONKEY),
    process.env.QNAMAKERAPIENDPOINTURL
  );
  const endpointKeys = await qnaMakerClient.endpointKeys.getKeys();
  const qnaServiceProvider = new QnaServiceProvider(
    configProvider,
    endpointKeys.primaryEndpointKey,
    process.env.QNAMAKERHOSTURL
  );
