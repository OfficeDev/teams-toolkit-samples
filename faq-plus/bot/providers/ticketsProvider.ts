...
export class TicketsProvider {
  ...
  private async storeOrUpdateTicketEntity(entity: TicketEntity): Promise<void> {
    ...
    return new Promise((resolve, rejcet) => {
      this.tableService.insertOrReplaceEntity(this.ticketTableName, entity, (error) => {
        if (!error) {
          resolve();
        }
        rejcet(error);
  ...
  private async ensureInitialize(): Promise<void> {
    return new Promise((resolve, reject) => {
      if (this.tableService) {
        ...
        this.tableService.createTableIfNotExists(this.ticketTableName, (error) => {
          if (!error) {
            resolve();
          }
          reject(error);
