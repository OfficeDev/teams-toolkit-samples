...
export class QnaServiceProvider {
  ...
  public async gGenerateAnswer(
    question: string,
    isTestKnowledgeBase: boolean,
    previousQnAId: string = null,
    previousUserQuery: string = null
  ): Promise<QnASearchResultList> {
    const knowledgeBaseId = await this.configurationProvider.getSavedEntityDetailAsync(
      ConfigurationEntityTypes.KnowledgeBaseId
    );
